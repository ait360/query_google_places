from create_excel import create_worksheet, create_workbook
from locality_dict import local_dictionary
from lga_dict import local_govt_area
from utils import is_running
from query_by_locality import get_state, get_state_locality
from query_by_lga import get_lga, get_state_lga



def help_search(google_api, action, api_key, local_dict, lga_dict, state=''):


    state_data = []

    is_running(get_state)

    state_data += get_state(google_api, action, api_key, state)

    is_running(get_state_locality)

    state_data = get_state_locality(local_dict, google_api,
                                    action, api_key, state_data,
                                    state)
    is_running(get_state_lga)

    state_data = get_state_lga(lga_dict, google_api, action,
                               api_key, state_data, state)

    is_running(get_lga)

    state_data = get_lga(lga_dict, google_api, action,
                         api_key, state_data, state)
    print('We found {} results for {}'.format(
                                        len(state_data), state))

    return state_data


def search(google_api, action, api_key, local_dict, lga_dict, state=''):



    if state:

        workbook = create_workbook(state)

        state_data = help_search(google_api, action, api_key, local_dict,
                                 lga_dict, state)

        create_worksheet(workbook, state_data, state)

    else:

        workbook = create_workbook()

        for state in local_dict.keys():

            state_data = help_search(google_api, action, api_key, local_dict,
                                     lga_dict, state)

            create_worksheet(workbook, state_data, state)


    workbook.close()
    print("Done, Closing the book")


local_dict = local_dictionary
lga_dict = local_govt_area


google_api = 'https://maps.googleapis.com/maps/api/place/textsearch/json?'


action = 'query='


api_key = '&key=AIzaSyArrc8-wt9ombcwu62LwbcWeb67k_u6oAg'

querybystate = input('Do you want to query a single state? y/n: ')


if querybystate == 'n':
    message = """You are querying all the states, this will take sometime\npress Ctrl+C to cancel"""

    print(message)

    search(google_api,action, api_key, local_dict, lga_dict)


elif querybystate == 'y':


    message = """\n\npick a state from the list below just as you can\nsee it in the list. Please do not add the quotes\n\n\n"""

    print(message)

    states = ['Abia', 'Abuja', 'Adamawa', 'Akwa+Ibom', 'Anambra',
              'Bauchi', 'Bayelsa', 'Benue', 'Bornu', 'Cross+river',
              'Delta', 'Ebonyi', 'Edo', 'Ekiti', 'Enugu', 'Gombe',
              'Imo', 'Jigawa', 'Kaduna', 'Kano', 'Katsina', 'Kebbi',
              'Kogi', 'Kwara', 'Lagos', 'Nasarawa', 'Niger', 'Ogun',
              'Ondo', 'Osun', 'Oyo', 'Plateau', 'Rivers', 'Sokoto',
              'Taraba', 'Yobe', 'Zamfara']

    print(states)

    state = input('May I have your state of interest: ')

    if state not in states:
        print('Please run the Script again following the instructions')
    else:
        search(google_api, action, api_key, local_dict, lga_dict, state)
